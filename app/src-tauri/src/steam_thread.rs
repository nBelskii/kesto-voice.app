...
pub fn init(shared: &Arc<SharedState>) {
    unsafe {
        ...
        // question we can't fully rule out. (We tried also logging
        // GetRelayNetworkStatus() for live diagnosis, but bindgen produces
        // an opaque, fieldless struct for SteamRelayNetworkStatus_t on the
        // Windows build — a platform difference in how it parses the
        // headers — so that part was dropped rather than fight it.)
